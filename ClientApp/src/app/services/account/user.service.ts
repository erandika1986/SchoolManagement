...
export class UserService {
  ...
  getUserList(name: string, roleId: number,
    currentPage: number, pageSize: number, sortBy: string, status: boolean): Observable<UserPaginatedItemsModel> {
    return this.httpClient.
      get<UserPaginatedItemsModel>(environment.apiUrl + 'User/GetUserList?name='
        + name + '&roleId=' + roleId + '&currentPage=' + currentPage + '&pageSize=' + pageSize + '&sortBy=' + sortBy + '&status=' + status);
  }
  ...
  getNewUser(): Observable<UserModel> {
    return this.httpClient.get<UserModel>(environment.apiUrl + 'User/GetNewUser');
  }
